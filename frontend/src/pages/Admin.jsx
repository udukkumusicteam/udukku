...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      ...
      if (error) {
        ...
        console.error('[admin] Failed to get session:', error);
      }
  ...
  useEffect(() => {
    ...
    // tables receives an INSERT / UPDATE / DELETE.
  ...
  const groups = useMemo(() => {
    ...
    arr.sort((a, b) =>
      a.formLabel.localeCompare(b.formLabel),
    );
  ...
  const onDelete = async (id) => {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error('[admin] Failed to delete submission:', err);
    }
  };

  const onExportGroup = (g) =>
    downloadCsv(g.formId, g.items);
  ...
  if (authLoading) {
    return (
      <main className="bg-cream min-h-screen flex items-center justify-center px-4">
        <div className="text-brown-mid text-sm">
  ...
  if (!session) {
    return (
      <main className="bg-cream min-h-screen flex items-center justify-center px-4">
        <section className="w-full max-w-md bg-white border border-brown-dark/10 rounded-3xl p-7 md:p-9 shadow-sm">
          <div className="text-center">
            <span className="uppercase tracking-[0.28em] text-xs text-brown-mid">
              Udukku
            </span>

            <h1 className="text-display mt-3 text-3xl md:text-4xl text-brown-dark">
              Admin Login
            </h1>
          ...
          >
            <div>
              <label className="block text-xs uppercase tracking-[0.18em] text-brown-mid mb-2">
                Email
              </label>
              ...
              <input
                ...
                autoComplete="email"
                required
                className="w-full h-11 px-4 rounded-full bg-cream/60 border border-brown-dark/10 text-sm text-brown-dark placeholder:text-brown-mid/60 focus:outline-none focus:border-orange"
              />
            </div>
            ...
            {loginError && (
              <div className="rounded-2xl bg-orange/10 border border-orange/20 px-4 py-3 text-sm text-orange">
                {loginError}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loggingIn}
              className="w-full h-11 rounded-full bg-brown-dark text-white text-sm font-medium hover:bg-orange transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {loggingIn
                ? 'Signing in…'
                : 'Sign In'}
      ...
      <section className="udukku-section pt-32 md:pt-40 pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <span
              className="text-brown-mid text-sm"
        ...
        <div className="mt-8 bg-white border border-brown-dark/10 rounded-3xl p-4 md:p-5 grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-4 items-center">
          ...
          <label className="md:col-span-2 flex items-center gap-2">
            <span className="text-brown-mid/80 text-xs">
              to
            </span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
              {entries.map(
                ([label, value]) => (
                  <div
                    key={label}
                    data-testid={`field-${submission.id}-${label}`}
                  >
                    ...
                    <div className="text-brown-dark text-sm leading-relaxed whitespace-pre-wrap break-words">
                      ...
                      value === null ||
                      value === '' ? (
                        <span className="text-brown-mid/50 italic">
                          Not provided
                        </span>
                      ) : (
                        String(
...
const downloadCsv = (
  ...
  a.href = url;
  a.download = `udukku-${formId}-${new Date()
    .toISOString()
    .slice(0, 10)}.csv`;
